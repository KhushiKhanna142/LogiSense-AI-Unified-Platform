    ...
    return (
        <div className='p-4'>
            <div className='flex items-center gap-2 mb-4'>
                ...
                <span className='text-xs text-gray-400'>Actor Agent — Carrier Swap Log</span>
            </div>

            {swaps.length === 0 && (
                <p className='text-xs text-gray-500'>No carrier swaps yet. Run demo_seed.py to trigger.</p>
            )}

            <div className='space-y-3'>
                {swaps.map((swap, i) => (
                    <div key={i} className='p-3 bg-teal-950 border border-teal-700 rounded-lg'
                        style={{ animation: i === 0 ? 'fadeIn 0.3s ease' : 'none' }}
                    >
                        <div className='flex justify-between items-center mb-1'>
                            <span className='text-teal-400 font-bold text-sm'>CARRIER SWAPPED</span>
                            <span className='text-xs text-gray-400 font-mono'>{swap.decision_id}</span>
                        </div>
                        <div className='text-sm text-white mb-1'>
                            <span className='text-red-400 font-mono'>{swap.old_carrier_id}</span>
                            <span className='text-gray-400 mx-2'>→</span>
                            <span className='text-green-400 font-mono'>{swap.new_carrier_id}</span>
                        </div>
